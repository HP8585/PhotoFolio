...
<div v-else class="py-7">
  <h1 class="text-center font-bold text-xl mb-6">{{ useRoute().params.id }}'s Followers</h1>
...
const apiKey = ref('ZPR4Gfo8YO3ch1BaL5XlTRBjow6uOuzzBSSeZZxTbBM')
const fetchUserFollowers = async(username)=>{
try{
    const res = await axios.get(`https://api.unsplash.com/users/${username}/followers?client_id=${apiKey.value}`) 
    if(res){
...
onMounted(()=>{
    ...
    log('This project is all made by HP(Hussain Panahy)')
})
</script>